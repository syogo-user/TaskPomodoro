...
class CardView: UIView, backgroundTimerDelegate {
    ...
    func setCurrentTimer(_ elapsedTime: Int) {
        // バックグラウンドでの経過時間を追加する(経過時間= elapsedTime)
        timer_sec += Float(elapsedTime)
        guard let task = self.task else { return }
        let timeValue = task.isBreak ? timeB : timeA
        if timeValue <= self.timer_sec {
            self.timer_sec = timeValue - 1
            // バックグランドで指定時間を超えた
            tooMuchTimeBackground = true
        }else{
            // フォアグラウンドで指定時間を超えた
            tooMuchTimeBackground = false
        }
        // 再びタイマーを起動
    ...
    private func setupLayout(task: TaskData) {
        ...
        startStopButton.anchor(top: pieChartView.bottomAnchor, centerX: centerXAnchor, width: 200,height: 100, topPadding: 10)
        baseStackView.anchor(top: cardImageView.topAnchor, left: cardImageView.leftAnchor, right: cardImageView.rightAnchor, topPadding: 60, leftPadding: 20, rightPadding: 20)
        goodLabel.anchor(top: cardImageView.topAnchor, left: cardImageView.leftAnchor, width: 140, height: 55, topPadding: 10 , leftPadding: 20)
        nopeLabel.anchor(top: cardImageView.topAnchor, right: cardImageView.rightAnchor, width: 140, height: 55, topPadding: 10, rightPadding: 20)
        ...
        let hour = timeLeft / 60
        let minutes = timeLeft.truncatingRemainder(dividingBy: 60.0)
        self.timerLabel.text = String(format: "%02d", Int(hour)) + " : " + String(format: "%02d", Int(minutes))
        // 円の表示
